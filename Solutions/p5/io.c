...
do_dev_word(struct event *ep)
{
	...
	// if not, add 100 usec to event and repeat = 1, put back on tq
	// if so, free io entry & call devtab[r0].write(r1) equiv

	if (iop->rw == IO_READ) {
		if (devtab[iop->device].rcheck()) {
			ch = devtab[iop->device].read();
			//*(iop->data) = rtn;
			ep->repeat_interval = 0;
			ep->max_repeats = 0;
			freedid = iop->threadid;
			free_io_entry(iop);
			wake_thread(freedid, ch);
		} else {
			ep->repeat_interval = MSEC(10);
			ep->max_repeats = 1;
		}
	} else {
		if (devtab[iop->device].wcheck()) {
