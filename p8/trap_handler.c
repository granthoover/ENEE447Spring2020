...
trap_handler(unsigned long r0, unsigned long r1, unsigned long r2)
{
	...
	switch (r7) {
		...
		case SYSCALL_RD_CBUF:
			...
			if (r0 < MAX_DEVICES) {
				return devtab[r0].read((char *)vm_translate(thread_activeid(), r1), r2);
			}
			break;
		case SYSCALL_WR_CBUF:
			...
			debug(DEBUG_LOW, "SYSCALL_RD_CBUF siz =", r2);
			if (r0 < MAX_DEVICES) {
				return devtab[r0].write((char *)vm_translate(thread_activeid(), r1), r2);
			}
			break;
	...
	log("PANIC Unknown System Call ", r7);
